...
def load_approved_ciphers(csv_file='approved_ciphers.csv'):
    ...
    CSV format: cipher_name,protocol,rating,format
    ...
    try:
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                ...
                APPROVED_CIPHERS[key] = (rating, cipher_name, cipher_format)
...
def check_cipher_compliance(cipher, protocol):
    ...
    Returns: (rating, cipher_name, format) where rating is 'RECOMMENDED' or 'SECURE' or 'NOT_APPROVED'
    """
    # Look up using both cipher name and protocol
    key = (cipher, protocol)
    if key in APPROVED_CIPHERS:
        rating, cipher_name, cipher_format = APPROVED_CIPHERS[key]
        return (rating, cipher_name, cipher_format)
    ...
    for tls_name, result in results.items():
        ...
        if status == 'SUPPORTED':
            ...
            for cipher in result.get('ciphers', []):
                compliance_rating, matched_cipher, cipher_format = check_cipher_compliance(cipher, result.get('protocol', ''))
                protocol_info['ciphers'].append({
                    'name': cipher,
        ...
        if args.json:
            ...
            for tls_name in TLS_VERSIONS.keys():
                ...
                if status == 'SUPPORTED':
                    ...
                    print(f"  Ciphers ({len(ciphers)}):")
                    for cipher in ciphers:
                        compliance_rating, cipher_name, cipher_format = check_cipher_compliance(cipher, result.get('protocol', ''))
                        status_icon = '✅' if compliance_rating == 'RECOMMENDED' else '⚠️ ' if compliance_rating == 'SECURE' else '❓'
